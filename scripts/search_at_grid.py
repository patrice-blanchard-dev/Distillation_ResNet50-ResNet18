"""Run a deterministic grid search for Attention Transfer settings."""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Grid search for Attention Transfer (AT) on CIFAR-100')
    ...
    parser.add_argument('--save_subdir', type=str, default='at_grid')
    ...
    parser.add_argument('--wandb_tags', type=str, default='at,search,grid')
    parser.add_argument('--wandb_mode', type=str, default='online', choices=['online', 'offline', 'disabled'])
    return parser.parse_args()


QUICK_GRID: Dict[str, List[Any]] = {
    'at_beta': [5.0, 10.0, 25.0, 50.0],
    'lr': [0.05, 0.08],
    'wd': [5e-4],
}

FULL_GRID: Dict[str, List[Any]] = {
    'at_beta': [2.5, 5.0, 10.0, 25.0, 50.0, 100.0],
    'lr': [0.03, 0.05, 0.08],
    'wd': [3e-4, 5e-4, 1e-3],
}
...
def cfg_key(cfg: Dict[str, Any]) -> Tuple[Any, ...]:
    return (cfg['at_beta'], cfg['lr'], cfg['wd'])
...
def build_command(args, exp_name: str, cfg: Dict[str, Any], idx: int) -> List[str]:
    ...
    if args.use_wandb:
        run_name = f"grid_{idx:03d}_b{cfg['at_beta']}_lr{cfg['lr']}_wd{cfg['wd']}"
        run_group = f'at-grid/{args.student_model}/{args.campaign_id}/{args.grid_preset}'
...
def main():
    ...
    tag = f'at_{args.student_model}_from_{args.teacher_model}_{args.save_subdir}'
    ...
    for rank, r in enumerate(results[:5], start=1):
        print(
            ...
            f"beta={r['at_beta']} | lr={r['lr']} | wd={r['wd']} | status={r.get('status')}"
        )

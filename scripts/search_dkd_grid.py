"""Run a deterministic grid search for DKD settings."""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Grid search for DKD on CIFAR-100')
    ...
    parser.add_argument('--save_subdir', type=str, default='dkd_grid')
    ...
    parser.add_argument('--wandb_tags', type=str, default='dkd,search,grid')
    parser.add_argument('--wandb_mode', type=str, default='online', choices=['online', 'offline', 'disabled'])
    return parser.parse_args()


QUICK_GRID: Dict[str, List[Any]] = {
    'dkd_temperature': [2.0, 4.0, 6.0],
    'dkd_alpha': [0.5, 1.0],
    'dkd_beta': [4.0, 8.0, 12.0],
    'lr': [0.05, 0.08],
    'wd': [5e-4],
}

FULL_GRID: Dict[str, List[Any]] = {
    'dkd_temperature': [2.0, 4.0, 6.0, 8.0],
    'dkd_alpha': [0.25, 0.5, 1.0, 1.5],
    'dkd_beta': [2.0, 4.0, 8.0, 12.0, 16.0],
    'lr': [0.03, 0.05, 0.08],
    'wd': [3e-4, 5e-4, 1e-3],
}
...
def cfg_key(cfg: Dict[str, Any]) -> Tuple[Any, ...]:
    return (cfg['dkd_temperature'], cfg['dkd_alpha'], cfg['dkd_beta'], cfg['lr'], cfg['wd'])
...
def build_command(args, exp_name: str, cfg: Dict[str, Any], idx: int) -> List[str]:
    cmd = [
        ...
        '--lr', str(cfg['lr']),
        '--wd', str(cfg['wd']),
        '--dkd_temperature', str(cfg['dkd_temperature']),
        '--dkd_alpha', str(cfg['dkd_alpha']),
        '--dkd_beta', str(cfg['dkd_beta']),
    ...
    if args.use_wandb:
        run_name = (
            f"grid_{idx:03d}_T{cfg['dkd_temperature']}_a{cfg['dkd_alpha']}_"
            f"b{cfg['dkd_beta']}_lr{cfg['lr']}_wd{cfg['wd']}"
        )
        run_group = f'dkd-grid/{args.student_model}/{args.campaign_id}/{args.grid_preset}'
...
def main():
    ...
    tag = f'dkd_{args.student_model}_from_{args.teacher_model}_{args.save_subdir}_{args.campaign_id}'

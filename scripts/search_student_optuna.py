"""Run an Optuna search for student baseline hyperparameters."""
...
TRAIN_STUDENT_MODULE = 'scripts.train_student'
...
def parse_args():
    parser = argparse.ArgumentParser(description='Optuna search for student baseline on CIFAR-100')
    ...
    parser.add_argument('--study_name', type=str, default='student_optuna')
    parser.add_argument('--storage', type=str, default='', help='Optional Optuna storage, e.g. sqlite:///student_optuna.db')
    parser.add_argument('--save_subdir', type=str, default='student_optuna', help='Subdirectory inside SAVE_DIR')
    ...
    parser.add_argument('--no_live_logs', action='store_true', help='Do not stream train_student output live in the terminal; only save it to subprocess.log')

    parser.add_argument('--use_wandb', action='store_true')
    parser.add_argument('--wandb_project', type=str, default='distill_cifar100')
    parser.add_argument('--wandb_entity', type=str, default='')
    parser.add_argument('--wandb_tags', type=str, default='student,search,optuna')
...
def _search_space(preset: str) -> Dict[str, Any]:
    if preset == 'quick':
        return {
            'lr': (0.045, 0.09),
            'wd': (5e-4, 1.2e-3),
            'label_smoothing_values': [0.1],
            'augmentations': ['mixup_cutmix'],
            'mixup_alpha': (0.15, 0.28),
            'cutmix_alpha': (0.9, 1.1),
        }
    return {
        'lr': (0.03, 0.12),
        'wd': (3e-4, 1.5e-3),
        ...
        'mixup_alpha': (0.12, 0.35),
        'cutmix_alpha': (0.8, 1.15),
    }
...
def build_command(args, exp_name: str, params: Dict[str, Any], trial_id: int) -> List[str]:
    ...
    if args.use_wandb:
        ...
        run_group = f'student-optuna/{args.study_name}/{args.model}/{args.campaign_id}/{args.search_preset}'
...
def _save_study_outputs(study: optuna.Study, args) -> Dict[str, str]:
    ...
    prefix = f'student_{args.model}_{args.save_subdir}_{args.study_name}_{stamp}'

"""Train the ResNet-50 teacher model on CIFAR-100."""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Training teacher on CIFAR-100')
    parser.add_argument('--model', type=str, default='resnet50_cifar')
    parser.add_argument('--gpu', type=int, default=0)
    parser.add_argument('--exp_name', type=str, default='default')
    parser.add_argument('--save_subdir', type=str, default='teacher')

    parser.add_argument('--epochs', type=int, default=EPOCHS_TEACHER)
    parser.add_argument('--lr', type=float, default=BASE_LR_TEACHER)
...
def _init_wandb(args, save_dir: str):
    ...
    run_name = args.wandb_run_name.strip() or f'teacher/{args.model}/{args.exp_name}'
...
def _log_best_checkpoint_artifact(wandb, args, save_dir: str, best_val_acc: Optional[float], test_acc: Optional[float], stopped_epoch: Optional[int]):
    ...
    artifact = wandb.Artifact(
        name=f'teacher-{args.model}-{args.exp_name}'.replace('/', '-'),
        type='model',
        metadata={
            'stage': 'teacher',
            'model': args.model,

"""Train a student with Attention Transfer distillation."""
...
from distillation.at import ATDistiller, parse_layer_names


def parse_args():
    parser = argparse.ArgumentParser(description='Train Attention Transfer (AT) student on CIFAR-100')
    ...
    parser.add_argument('--save_subdir', type=str, default='at')
    ...
    parser.add_argument('--at_beta', type=float, default=10.0)
    ...
    parser.add_argument('--wandb_tags', type=str, default='at')
...
def _init_wandb(args, save_dir: str):
    ...
    run_name = args.wandb_run_name.strip() or f'at/{args.student_model}/{args.exp_name}'
    ...
    wandb.init(
        ...
        config={
            ...
            'teacher_layers': parse_layer_names(args.teacher_layers),
            'student_layers': parse_layer_names(args.student_layers),
...
def _generate_curves(history: Dict[str, List[float]], save_dir: str) -> Tuple[str, str]:
    ...
    plt.plot(epochs, history['train_loss_at'], label='Train AT')
    if history['val_loss']:
        plt.plot(epochs, history['val_loss'], label='Val loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.title('AT Loss vs Epoch')
...
def _train_one_epoch_at(
    ...
    for images, labels in tqdm(dataloader, desc='Train AT', leave=False, dynamic_ncols=True):
        ...
        with _autocast_context(device, amp_enabled):
            ...
            logits_s = distiller.student_forward(images)

            if mixed:
                loss_ce = lam * ce_criterion(logits_s, y_a) + (1.0 - lam) * ce_criterion(logits_s, y_b)
            else:
                loss_ce = ce_criterion(logits_s, labels)

            loss_at = distiller.at_loss()
            loss = loss_ce + distiller.beta * loss_at
        ...
        if amp_enabled:
            ...
            if max_grad_norm is not None:
                ...
                torch.nn.utils.clip_grad_norm_(distiller.student.parameters(), max_grad_norm)
        ...
        total_at += float(loss_at.item()) * batch_size
        if mixed:
    ...
    return total_loss / denom, total_ce / denom, total_at / denom, 100.0 * total_correct / denom
...
def main():
    args = parse_args()
    if args.at_beta < 0:
        raise ValueError('--at_beta must be >= 0')
    teacher_layers = parse_layer_names(args.teacher_layers)
    student_layers = parse_layer_names(args.student_layers)
    ...
    ce_criterion = nn.CrossEntropyLoss(label_smoothing=float(args.hard_label_smoothing))
    distiller = ATDistiller(
        teacher=teacher,
        student=student,
        beta=float(args.at_beta),
        teacher_layers=teacher_layers,
        student_layers=student_layers,
    )

    history = {
        'epoch': [], 'train_loss': [], 'train_loss_ce': [], 'train_loss_at': [], 'train_acc': [], 'val_loss': [], 'val_acc': [], 'lr': []
    }
    ...
    try:
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            train_loss, train_ce, train_at, train_acc = _train_one_epoch_at(
                distiller=distiller,
            ...
            print(f'Train total: {train_loss:.4f} | CE: {train_ce:.4f} | AT: {train_at:.4f} | acc: {train_acc:.2f}')
            ...
            history['train_loss_at'].append(float(train_at))
        ...
        if wandb is not None:
            ...
            if args.wandb_log_artifacts:
                artifact = wandb.Artifact(
                    name=f'at-{args.student_model}-{args.exp_name}'.replace('/', '-'),
                    type='model',
                    metadata={
                        'stage': 'at',
                        'teacher_model': args.teacher_model,
